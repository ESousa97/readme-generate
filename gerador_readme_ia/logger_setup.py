...
from typing import Optional # Adicionado para Optional

# Removido APP_NAME daqui, será passado como parâmetro
LOG_FILE_NAME_DEFAULT = "app.log"

# Importar APP_AUTHOR de constants para usar como padrão
# Certifique-se de que constants.py não importe logger_setup.py para evitar importação circular.
# Se for o caso, APP_AUTHOR pode ser uma string literal aqui ou passada explicitamente.
try:
    from .constants import APP_AUTHOR as DEFAULT_APP_AUTHOR
except ImportError: # Fallback se constants.py não puder ser importado (ex: durante testes unitários de logger_setup)
    DEFAULT_APP_AUTHOR = "DefaultAppAuthor"
